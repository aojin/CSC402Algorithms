...
public class MyDequeUsingStacks<T> {
    ...
    public int size ()             { return sl.size() + sr.size(); } // size gives entire stack as one 
    public void pushLeft (T item)  { sl.push (item); } // only one left and right entrance
    ...
    private void move (ResizingArrayStack<T> from, ResizingArrayStack<T> to) {
        ...
        for (int i = 0; i < from.size()/2; i++) {
        	tmp.push(from.pop()); // slice the top half of from inverted into tmp
        }
        while(!from.isEmpty()) {
        	to.push(from.pop()); // take the rest of from into to, to expose item to return
        }
        while(!tmp.isEmpty()) {
        	from.push(tmp.pop()); //return top slice into from. Effectively balancing both sides of to and from
        }
        // StdOut.println ("sizes: " + from.size () + " " + tmp.size ());
    ...
    public T popLeft () {
        ...
        if (sl.isEmpty ()) { move (sr, sl); } // move used to take entire contents of right and push to sl
        return sl.pop (); // move only engages if stack side .isEmpty
    }

    public T popRight () {
        if (isEmpty()) { throw new NoSuchElementException(); }
        if (sr.isEmpty ()) { move (sl, sr); } // move used to take entire contents of left and push to sr
        return sr.pop ();
    }
